...
async function getProject(projectId: string) {
  ...
  if (error || !data) throw new Error(error?.message || `Project ${projectId} not found`)
...
async function ensureProjectAndPrdItem(idea: Record<string, unknown>) {
  ...
  if (!project) {
    ...
    const base = String(idea.title || 'project')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '')
      .slice(0, 40) || 'project'
  ...
  if (!items || items.length === 0) {
    ...
    if (error) throw error
    ;({ data: items, error: itemError } = await supabase
      .from('backlog_items')
      .select('*')
      .eq('project_id', project.id)
      .eq('title', prdTitle))
  ...
  return {
    project: assert(project, 'project missing'),
    prdItem: assert(items?.[0] as BacklogItem | undefined, 'prd item missing'),
    createdProject,
  }
}
...
async function executeAssignedItem(item: BacklogItem, project: Record<string, unknown>, finalStatus: 'review' | 'done') {
  ...
  try {
    ...
    const generated = await generateProjectArtifactWithHermes({
      artifact: claimed.stage === 'prd' ? 'prd' : claimed.stage === 'planning' ? 'planning' : 'sprint-review',
      agent,
      project: {
        ...
        completedTasks: deps.map((dep) => ({
          title: String(dep.title || ''),
          assignee_slug: (dep.assignee_slug as string | null) || null,
          artifact_markdown: (dep.artifact_markdown as string | null) || null,
        })),
      },
    })
    await markSuccess(claimed.id, generated.content, finalStatus)
    report.actions.push({
...
async function maybePrepareSprintReview(project: Record<string, unknown>) {
  ...
  await supabase
    .from('project_sprints')
    .update({ status: 'in_review', review_markdown: generated.content, completed_at: t, updated_at: t })
    .eq('project_id', project.id)
    .eq('sprint_number', sprintNumber)
  ...
  if (idea) {
    await supabase
      .from('business_ideas')
      .update({ workflow_stage: 'sprint_review', automation_status: 'needs_feedback', review_requested_at: t })
      .eq('id', idea.id)
  }
...
async function main() {
  ...
  for (const idea of prdIdeas || []) {
    ...
    try {
      ...
      await supabase
        .from('projects')
        .update({
          prd_markdown: artifact,
          prd_status: 'draft',
          prd_generated_at: t,
          prd_generated_by: 'product-lead',
  ...
  if (processed < MAX_TASKS) {
    const { data: planningProjects, error } = await supabase
      .from('projects')
      .select('*')
      .eq('prd_status', 'approved')
      .eq('execution_status', 'planning_generation')
    if (error) throw error
      ...
      try {
        ...
        await supabase
          ...
          .update({
            ...
            planning_generated_at: t,
            planning_generated_by: 'hermes',
        ...
        if (idea) {
          await supabase
            .from('business_ideas')
            .update({ workflow_stage: 'planning_review', automation_status: 'needs_feedback', review_requested_at: t })
            .eq('id', idea.id)
        }
  ...
  if (processed < MAX_TASKS) {
    const { data: sprintReadyProjects, error } = await supabase
      .from('projects')
      .select('*')
      .eq('execution_status', 'sprint_ready')
      .gte('current_sprint_number', 1)
    if (error) throw error
    ...
    for (const project of sprintReadyProjects || []) {
      ...
      await supabase
        .from('project_sprints')
        .update({ status: 'in_progress', started_at: t, updated_at: t })
        .eq('project_id', project.id)
        .eq('sprint_number', sprintNumber)
      await supabase
        .from('projects')
        .update({ execution_status: 'sprint_in_progress', delivery_status: 'sprint_execution', updated_at: t })
        .eq('id', project.id)
      const idea = await getIdeaByProject(String(project.id))
      if (idea) {
        await supabase
          .from('business_ideas')
          .update({ workflow_stage: 'sprint_execution', automation_status: 'running' })
          .eq('id', idea.id)
      }
  ...
  if (processed < MAX_TASKS) {
    const { data: activeProjects, error } = await supabase
      .from('projects')
      .select('*')
      .eq('execution_status', 'sprint_in_progress')
    if (error) throw error
